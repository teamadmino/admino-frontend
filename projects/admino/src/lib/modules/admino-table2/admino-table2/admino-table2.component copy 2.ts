...
export class AdminoTable2Component implements OnInit, AfterViewInit, OnDestroy {
    ...
    switch (event.key) {
      ...
      case "PageDown":
        let goalDown =
          ...
          this.visibleRowCount +
          (this.activeRow - this.rowStart);
        if (goalDown > this.totalsize - 1) {
          goalDown = this.totalsize - 1;
        }
        this.gotoPos(goalDown);
        this.activeRow = goalDown;
        event.preventDefault();
        break;
      case "Enter":
        this.setSelected(
  ...
  scrollEvent() {
    ...
    if (
      ...
      this.scrollPos = this.tableRef.nativeElement.scrollTop =
        this.largePage <= 1 ? this.rowHeight : overshoot;
  ...
  updateSize() {
    ...
    this.calculatedLargePageSize = 15;
